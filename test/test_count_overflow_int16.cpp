...
int main() {
    ...
    for (int size = 128; size < 32768; size <<= 1) {
        ...
        cudaSafeCall(cudaMemcpy(d_array, h_array, size * size * sizeof(int16_t), cudaMemcpyHostToDevice));
        ...
        for (int i = 0; i < 128; i++)
            ape::count_overflow_int16emu(d_array, size, size);

        float ms;
        cudaEventRecord(st);
        for (int i = 0; i < 128; i++)
            d_count = ape::count_overflow_int16emu(d_array, size, size);
        cudaEventRecord(ed);
        ...
        std::cout << size << "x" << size << ": " << size * size * sizeof(int16_t) * 128.0 / (ms * 1024.0 * 1024.0) << " GB/s"
                  << std::endl;
        cudaFree(d_array);
    }
