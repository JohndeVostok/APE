...
namespace ape {
__global__ void kernel_create_mask_a_fp32(const float *src, size_t m, size_t k, ApeTrans transa,
                                               int8_t *mask) {
...
void create_mask_a_fp32(const float *src, size_t m, size_t k, ApeTrans transa,
                                               int8_t *mask) {
    dim3 grid((m - 1) / AUTO_BLOCK + 1, (k - 1) / AUTO_CHUNK + 1, 1);
    dim3 block(1, AUTO_CHUNK, 1);

    kernel_create_mask_a_fp32<<<grid, block, ((AUTO_CHUNK - 1)/32+1) * sizeof (int8_t)>>>(src, m, k, transa, mask);

    cudaCheckError();
}


__global__ void kernel_create_mask_b_fp32(const float *src, size_t k, size_t n, ApeTrans transb,
                                               int8_t *mask) {
    extern __shared__ char smem[];
    int8_t *shmem = (int8_t *)&smem[0];
    uint32_t warp_id = threadIdx.y / 32;
...
void create_mask_b_fp32(const float *src, size_t k, size_t n, ApeTrans transb,
                                               int8_t *mask) {
    dim3 grid((n - 1) / AUTO_BLOCK + 1, (k - 1) / AUTO_CHUNK + 1, 1);
    dim3 block(1, AUTO_CHUNK, 1);

    kernel_create_mask_b_fp32<<<grid, block, ((AUTO_CHUNK - 1)/32+1) * sizeof (int8_t)>>>(src, k, n, transb, mask);
