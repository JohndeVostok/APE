...
void create_mask_a_fp32(const float *src, size_t m, size_t k, ApeTrans transa,
    ...
    kernel_create_mask_a_fp32<<<grid, block, ((AUTO_CHUNK - 1)/32+1) * sizeof (int)>>>(src, m, k, transa, mask);
...
void create_mask_b_fp32(const float *src, size_t k, size_t n, ApeTrans transb,
    ...
    kernel_create_mask_b_fp32<<<grid, block, ((AUTO_CHUNK - 1)/32+1) * sizeof (int)>>>(src, k, n, transb, mask);
